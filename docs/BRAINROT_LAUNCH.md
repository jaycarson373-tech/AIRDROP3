...
2. Confirm the official $NEURALP mint is configured as
...
5. Set `NEXT_PUBLIC_STONK_URL` to the exact live StonkFun token page and verify
   the launch is paired with the intended `$NEURALP` asset.
...
8. Confirm the $BRAINROT and $NEURALP mints resolve to supported SPL token
