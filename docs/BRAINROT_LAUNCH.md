...
2. Confirm the official $NEURALINK mint is configured as
...
5. Set `NEXT_PUBLIC_STONK_URL` to the exact live StonkFun token page and verify
   the launch is paired with the intended `$NEURALINK` asset.
...
8. Confirm the $BRAINROT and $NEURALINK mints resolve to supported SPL token
