...
Cat Strategy is a live cat-runner reward site for `CSTR`. Eligible holders receive the configured cat-token runner on the live epoch cadence.
...
- `/terminal` - live Cat Strategy dashboard and active cat-runner status
...
3. Configure the Cat Strategy source mint, Supabase, RPC, treasury secrets, and active `CAT` reward mint.
4. Confirm the public site shows the Cat Strategy CA, X community link, buy link, reward basket, and no stale project links.
