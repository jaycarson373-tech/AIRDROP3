...
async function tokenBalance(wallet: string) {
  ...
  if (!mint) throw new Error("CAT STRATEGY token mint is not configured");
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (balance < minimum) {
      return NextResponse.json({ error: "Wallet is below the CAT STRATEGY access threshold", balance, minimum }, { status: 403 });
    }
    ...
    console.error("CAT STRATEGY access verification failed", error);
