...
async function tokenBalance(wallet: string) {
  ...
  if (!mint) throw new Error("CASINO STRATEGY token mint is not configured");
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (balance < minimum) {
      return NextResponse.json({ error: "Wallet is below the CASINO STRATEGY access threshold", balance, minimum }, { status: 403 });
    }
    ...
    console.error("CASINO STRATEGY access verification failed", error);
