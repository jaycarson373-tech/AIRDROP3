...
async function runCommand(message: TelegramMessage, name: string, args: string, premium: boolean) {
  ...
  if (name === "runner") {
    ...
    return active ? formatSignalTelegram(active, premium) : "<b>RUNNER</b>\nAwaiting the first active signal.";
  }
  if (name === "top") {
    const signals = (await listScoutSignals({ premium, limit: 30 }))
      .sort((a, b) => Number(b.scout_score ?? -1) - Number(a.scout_score ?? -1));
    return shortSignalList(signals, "TOP RUNNER SIGNALS");
  }
  if (name === "new") {
    return shortSignalList(await listScoutSignals({ premium, limit: 8 }), "NEW RUNNER SIGNALS");
  }
  if (name === "search") {
  ...
  if (name === "performance") {
    ...
    return shortSignalList(signals, "RUNNER SELECTION HISTORY");
  }
  if (name === "help") {
    return [
      "<b>RUNNER COMMANDS</b>",
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("Runner Telegram webhook failed", error);
    return NextResponse.json({ ok: true });
  }
}
