...
async function supabaseRequest<T>(
  ...
  if (!config) throw new Error("SNDK6900 database is not configured");
  ...
  if (!response.ok) {
    ...
    throw new Error(`SNDK6900 database request failed (${response.status}): ${detail.slice(0, 400)}`);
  }
...
export async function ingestScoutSignal(input: SignalInput) {
  ...
  const activated = await activateScoutSignal(
    signal.id,
    input.forceActivate ? "Administrator override" : current ? `Score advantage ${scoreAdvantage}` : "First active SNDK6900 distribution asset"
  );
...
export function formatSignalTelegram(signal: ScoutSignal, premium: boolean) {
  ...
  return [
    `<b>${premium ? "SNDK6900 EARLY DISTRIBUTION ASSET" : "SNDK6900 PUBLIC DISTRIBUTION ASSET"}</b>`,
