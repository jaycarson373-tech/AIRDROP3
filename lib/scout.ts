...
async function supabaseRequest<T>(
  ...
  if (!config) throw new Error("CASINO STRATEGY database is not configured");
  ...
  if (!response.ok) {
    ...
    throw new Error(`CASINO STRATEGY database request failed (${response.status}): ${detail.slice(0, 400)}`);
  }
...
export async function ingestScoutSignal(input: SignalInput) {
  ...
  const activated = await activateScoutSignal(
    signal.id,
    input.forceActivate ? "Administrator override" : current ? `Score advantage ${scoreAdvantage}` : "First active CASINO STRATEGY settlement asset"
  );
...
export async function createAccessChallenge(wallet: string) {
  ...
  const message = `CASINO STRATEGY access\nWallet: ${normalized}\nNonce: ${nonce}\nExpires: ${expiresAt}`;
...
export function formatSignalTelegram(signal: ScoutSignal, premium: boolean) {
  ...
  return [
    `<b>${premium ? "CASINO STRATEGY EARLY SETTLEMENT ASSET" : "CASINO STRATEGY PUBLIC SETTLEMENT ASSET"}</b>`,
