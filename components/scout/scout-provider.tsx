...
export function ScoutProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : "BUFFETTCOIN data is unavailable");
    }
  }, [accessToken]);
  ...
  const unlockScout = useCallback(async () => {
    ...
    try {
      ...
      setAccessError(nextError instanceof Error ? nextError.message : "BUFFETTCOIN access could not be verified");
    } finally {
      setAccessBusy(false);
    }
  }, [lookupWallet]);
