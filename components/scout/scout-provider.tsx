...
export function ScoutProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : "CAT STRAT data is unavailable");
    }
  }, [accessToken]);
  ...
  const unlockScout = useCallback(async () => {
    ...
    try {
      ...
      setAccessError(nextError instanceof Error ? nextError.message : "CAT STRAT access could not be verified");
    } finally {
      setAccessBusy(false);
    }
  }, [lookupWallet]);
